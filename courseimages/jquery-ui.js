...
/*Placeholder for scaled objects fixed by WebSoft 160912 */
/*Scaled drag/drop fixes by WebSoft 161023 */
...
$.extend( $.effects, {
	...
	// modified by WebSoft to fix positioning on scaled objects
	createPlaceholder: function( element ) {
		var placeholder, 
			cssPosition = element.css( "position" ),
			position = element.position(), // position returns wrong (scaled) values
		...
		{
			var oBounds = element[0].getBoundingClientRect();
			if(oBounds.width!=ew)
			{
				zx = oBounds.width/ew;
			}
			if(oBounds.height!=eh)
			{
				zy = oBounds.height/eh;
			}
			x = position.left/zx;
			y = position.top/zy;
		}
...
$.widget( "ui.draggable", $.ui.mouse, {
	...
	_setContainment: function() {
		...
		this.containment = [
			CLZ.nZoom*(( parseInt( c.css( "borderLeftWidth" ), 10 ) || 0 ) + ( parseInt( c.css( "paddingLeft" ), 10 ) || 0 )),
			CLZ.nZoom*(( parseInt( c.css( "borderTopWidth" ), 10 ) || 0 ) + ( parseInt( c.css( "paddingTop" ), 10 ) || 0 )),
			CLZ.nZoom*(( isUserScrollable ? Math.max( ce.scrollWidth, ce.offsetWidth ) : ce.offsetWidth ) - ( parseInt( c.css( "borderRightWidth" ), 10 ) || 0 ) - ( parseInt( c.css( "paddingRight" ), 10 ) || 0 ) - this.helperProportions.width - this.margins.left - this.margins.right),
			CLZ.nZoom*(( isUserScrollable ? Math.max( ce.scrollHeight, ce.offsetHeight ) : ce.offsetHeight ) - ( parseInt( c.css( "borderBottomWidth" ), 10 ) || 0 ) - ( parseInt( c.css( "paddingBottom" ), 10 ) || 0 ) - this.helperProportions.height - this.margins.top - this.margins.bottom)
		];// COURSELAB: here's correction for scaled dragging
	...
	_generatePosition: function( event, constrainPosition ) {
		...
		return {
			top: ( pageY - this.offset.click.top - this.offset.relative.top - this.offset.parent.top + ( this.cssPosition === "fixed" ? -this.offset.scroll.top : ( scrollIsRootNode ? 0 : this.offset.scroll.top ) ))/CLZ.nZoom,
			left: ( pageX - this.offset.click.left - this.offset.relative.left - this.offset.parent.left + ( this.cssPosition === "fixed" ? -this.offset.scroll.left : ( scrollIsRootNode ? 0 : this.offset.scroll.left )))/CLZ.nZoom
		};// COURSELAB: here's correction for scaled dragging
...
$.ui.ddmanager = {
	...
	prepareOffsets: function( t, event ) {
		...
		droppablesLoop: for ( i = 0; i < m.length; i++ ) {
			...
			m[ i ].proportions( {
				width: m[ i ].element[ 0 ].offsetWidth*CLZ.nZoom,
				height: m[ i ].element[ 0 ].offsetHeight*CLZ.nZoom
			} ); // COURSELAB: here's correction for droppable area size
